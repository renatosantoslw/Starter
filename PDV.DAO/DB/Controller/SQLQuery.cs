...
namespace PDV.DAO.DB.Controller
{
    public class SQLQuery : IDisposable
    {
        ...
        private int UpdateDataAdapter(DataTable dt)
        {
            ...
            return ((SqlDataAdapter)objDataAdapter).Update(dt);
        }
        #endregion
        ...
        public int ExecSQL()
        {
            objCommand = new SqlCommand(SQL, (SqlConnection)DBUtils.CONTROLADOR.Pool[IDConexao.HasValue ? IDConexao.Value : DBUtils.IDCONEXAO_PRIMARIA].Connection);
        ...
        public void Open()
        {
            ...
            try
            {
                ...
                objDataAdapter = new SqlDataAdapter(objCommand as SqlCommand);


                //objCo//((NpgsqlDataAdapter)objDataAdapter).Fill(dtDados);mmandBuilder = new NpgsqlCommandBuilder((NpgsqlDataAdapter)objDataAdapter);
                objCommandBuilder = new SqlCommandBuilder((SqlDataAdapter)objDataAdapter);

                
                
                ((SqlDataAdapter)objDataAdapter).Fill(dtDados);
